...
export const WAVE_PLANS=[null,null,[8,3,78,4],[10,3,88,3.5],[14,3,106,2.8],[20,4,138,2.2],[27,4,160,2.1],[32,4,180,2],[36,4,194,1.9],[38,4,216,2],[40,5,220,1.8],[43,5,232,1.7],[46,5,244,1.6],[49,5,254,1.5],[52,5,264,1.45],[55,6,274,1.4],[58,6,284,1.35],[62,6,294,1.3],[66,6,306,1.25],[70,6,330,1.3],[20,3,56,1.3]];
export function makeWaves(stage,seed){if(stage.id===1)return [];let rand=rng(seed),[count,groups,last,gap]=WAVE_PLANS[stage.id],first=stage.id<6?18:22,list=[],done=0;let types=stage.types.filter(t=>t!=='armored');for(let g=0;g<groups;g++){let size=g===0?Math.min(4,Math.floor(count/groups)):g===groups-1?count-done:Math.floor((count-done)/(groups-g)),start=first+(last-first)*g/(groups-1);for(let i=0;i<size;i++){let lane=stage.id<=3&&g===0?2:stage.id<=5&&g===0?[1,2,3][i%3]:(i+g)%5;let type=types[Math.floor(rand()*types.length)];if(g===0)type=types.includes('normal')?'normal':types.includes('cold')?'cold':types.includes('office')?'office':[...types].filter(t=>!['complaint','delivery','bulky'].includes(t)).sort((a,b)=>(E[a].hp+(E[a].shield||0))-(E[b].hp+(E[b].shield||0)))[0]||types[0];if(type==='bulky'&&i%3!==0)type=types.find(t=>t!=='bulky')||type;if(['delivery','complaint','bulky'].includes(type)&&i%3!==0)type=[...types].filter(t=>!['delivery','complaint','bulky'].includes(t)).sort((a,b)=>(E[a].hp+(E[a].shield||0))-(E[b].hp+(E[b].shield||0)))[0]||type;if(stage.id===9&&g===1)lane=2;if(stage.id===5&&g===groups-1)type='office';list.push({at:start+i*gap,lane,type,group:g});done++;}}
